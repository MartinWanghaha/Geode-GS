...
class GaussianModel:
    ...
    def densification_postfix(self, new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii):
        d = {"xyz": new_xyz,
        "knn_f": new_knn_f,
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_tmp_radii)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition 选出需要 densify 的高斯球
        # 梯度大意味着该区域“学习压力”大，模型没拟合好；用 L2 范数筛选出这些“重点区域”的高斯球。
        selected_pts_mask = torch.where(torch.norm(grads, dim=-1) >= grad_threshold, True, False)
        # 再进一步筛掉太大的点，避免在太大的高斯上 densify
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        if selected_pts_mask.sum() + n_init_points > self.max_all_points: # 控制新增点数量
            limited_num = self.max_all_points - n_init_points
        ...
        if selected_pts_mask.sum() > 0:
            ...
            self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii)

    def densify_and_prune(self, max_grad, abs_max_grad, min_opacity, extent, max_screen_size, radii):
