...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        for resolution_scale in resolution_scales:
            ...
            all_cameras_at_scale = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale, args)
            N = len(all_cameras_at_scale) # 获取当前分辨率下所有相机的总数
            self.test_cameras[resolution_scale] = [all_cameras_at_scale[idx] for idx in range(0, N) 
                                               if idx % 8 == 0]              
            self.train_cameras[resolution_scale] = [all_cameras_at_scale[idx] for idx in range(0, N) 
                                               if idx % 8 != 0]            
