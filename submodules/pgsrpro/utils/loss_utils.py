...
def lncc(ref, nea):
    # ref_gray: [batch_size, total_patch_size] ref: 参考图像的 patch，形状为 [batch_size, total_patch_size]
    # nea_grays: [batch_size, total_patch_size] nea: 需要与参考图像对比的 patch，形状同上
    bs, tps = nea.shape # 获取 batch 大小 和 每个 patch 的像素数量（总像素数）
    patch_size = int(np.sqrt(tps)) # 假设 patch 为正方形，计算边长

    ref_nea = ref * nea # 计算像素级乘积（用于协方差计算）
    ref_nea = ref_nea.view(bs, 1, patch_size, patch_size) # 变形为 [B, 1, H, W]
    ref = ref.view(bs, 1, patch_size, patch_size)
    nea = nea.view(bs, 1, patch_size, patch_size)
    ref2 = ref.pow(2) # 参考图像的平方
    nea2 = nea.pow(2) # 待比较图像的平方

    # sum over kernel 创建一个全为 1 的卷积核，用于做局部求和
    filters = torch.ones(1, 1, patch_size, patch_size, device=ref.device)
    padding = patch_size // 2 

    # 对每个 patch 执行求和操作（卷积相当于窗口内所有值求和）
    ...
    # average over kernel # 求局部平均值
    ref_avg = ref_sum / tps # 参考图像平均值
    nea_avg = nea_sum / tps # 对比图像平均值
    # 协方差项
