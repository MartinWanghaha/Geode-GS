...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scale_path = os.path.join(scene.model_path, "scale")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if 'app_image' in render_pkg and ssim_loss < 0.5:
            app_image = render_pkg['app_image'] # 在实际场景中会出现浮动的噪点, 通过render为每个图像app_image添加两个曝光系数a,b
        ...
        if visibility_filter.sum() > 0:
            scale = gaussians.get_scaling[visibility_filter]  # (N_visible, 3)
            sorted_scale, _ = torch.sort(scale, dim=-1)  # 每个高斯三个轴向
            min_scale = sorted_scale[..., 0]
            max_scale = sorted_scale[..., -1]          
            # 限制最小尺度不要过小，最大尺度不要过大
            min_threshold = opt.scale_min_thresh  # 比如 0.01
            max_threshold = opt.scale_max_thresh  # 比如 0.2
            # 防止过小：小于 min_threshold 的惩罚
            small_scale_penalty = torch.relu(min_threshold - min_scale)
            # 防止过大：大于 max_threshold 的惩罚
            large_scale_penalty = torch.relu(max_scale - max_threshold)
            scale_loss = small_scale_penalty.mean() + large_scale_penalty.mean()
            loss += opt.scale_loss_weight * scale_loss
            print(f"min_scale: min={min_scale.min().item():.4f}, max={min_scale.max().item():.4f}, mean={min_scale.mean().item():.4f}, std={min_scale.std().item():.4f}")
            print(f"max_scale: min={max_scale.min().item():.4f}, max={max_scale.max().item():.4f}, mean={max_scale.mean().item():.4f}, std={max_scale.std().item():.4f}")
            if iteration > 10000 and iteration % 50 == 0:
                import matplotlib.pyplot as plt
                plt.hist(min_scale.detach().cpu().numpy(), bins=50, alpha=0.5, label='min_scale')
                plt.hist(max_scale.detach().cpu().numpy(), bins=50, alpha=0.5, label='max_scale')

                plt.axvline(min_threshold, color='r', linestyle='--', label='min_threshold')
                plt.axvline(max_threshold, color='g', linestyle='--', label='max_threshold')
                ...
                save_name = "%05d_%s.jpg" % (iteration, viewpoint_cam.image_name)
                save_path = os.path.join(scale_path, save_name)
                plt.savefig(save_path)
                plt.close()  # 清空画布，防止内存爆涨
        ...
        if iteration > opt.single_view_weight_from_iter:
            ...
            # ---------- prior loss ----------
                ...
                # prior_normal = viewpoint_cam.normal_prior.to('cuda') * (render_alpha).detach()
                prior_normal = viewpoint_cam.normal_prior.to('cuda')
                if not opt.wo_image_weight:
                    normal_loss = weight * (image_weight * (normal - prior_normal).abs().sum(0)).mean()
                else:
                    normal_loss = weight * ((normal - prior_normal).abs().sum(0)).mean()
                depth_normal = prior_normal
                loss += (normal_loss)
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                ncc_weight = opt.multi_view_ncc_weight # 从MVS中获得灵感，多视角光度loss
                # 取像素周围7x7的像素块，将其映射到相邻帧中，并将其转换成灰度图，使用NCC（normalized cross correlation）计算两个patch之间的光度误差

                geo_weight = opt.multi_view_geo_weight # 重投影误差，多视角几何loss
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                mask = (render_pkg["out_observe"] > 0) & visibility_filter # 更新每个高斯球的最大投影半径
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0: # 控制 densify 的触发频率
                    # 一个高斯在图像中占的像素太小（< 20px），可以直接删掉
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    # 如果某点附近误差/梯度 > grad_threshold，在此区域复制出更多小高斯球
