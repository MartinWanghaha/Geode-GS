...
from utils.graphics_utils import patch_offsets, patch_warp, guided_normal_fusion
    ...
    def train_step_gs(self, max_lr = False, no_densify = False):
        ...
        if self.training_step > self.training_config.single_view_weight_from_iter:
            ...
            # ---------- prior loss ----------
                ...
                if self.training_step % 200 == 0: # 存图可视化
                    ...
                    row1 = np.concatenate([depth_color, depth_normal_show, fusion_normal_show, image_weight_color], axis=1)
        ...
        with torch.no_grad():
            if self.gs_step < self.training_config.densify_until_iter and not no_densify:
                ...
                # self.GS.gaussians.add_densification_stats(render_pkg["viewspace_points"], render_pkg["visibility_filter"])
                self.GS.gaussians.add_densification_stats(render_pkg["viewspace_points"], render_pkg["viewspace_points_abs"], render_pkg["visibility_filter"])
        ...
        if self.gs_step < self.training_config.densify_until_iter:
            if (self.gs_step > self.training_config.densify_from_iter) and \
                ...
                self.GS.gaussians.densify_and_prune(self.training_config.densify_grad_threshold,
                                                    self.training_config.densify_abs_grad_threshold,
                                                    self.training_config.opacity_cull_threshold,
